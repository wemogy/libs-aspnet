...
using Wemogy.AspNetCore.Exceptions;
using Wemogy.Core.Extensions;
using Wemogy.Core.Json;
using Wemogy.DataAccess.Context;
    ...
    public abstract class ApiGatewayBase: ControllerBase
    {
        ...
        private ICollection<KeyValuePair<string, string>> GetHeadersMapped()
        {
            ...
            if (this._httpContextAccessor.HttpContext.User.Identity != null && this._httpContextAccessor.HttpContext.User.Identity.IsAuthenticated)
            {
                ...
                {
                    headers.Add(HeaderContext.SubjectIdHeaderKey, nameIdentifier);
                }
            }

            if (this._httpContextAccessor.HttpContext.Request.Headers.ContainsKey(HeaderContext.TenantIdHeaderKey))
            {
                var tenantIdHeader = this._httpContextAccessor.HttpContext.Request.Headers[HeaderContext.TenantIdHeaderKey].FirstOrDefault();
                // ensure that tenant id is a GUID
                if (Guid.TryParse(tenantIdHeader, out Guid tenantId))
                {
                    headers.Add(HeaderContext.TenantIdHeaderKey, tenantId.ToString());
                }
                else
                {
                    throw new InvalidHeaderException(HeaderContext.TenantIdHeaderKey, tenantIdHeader);
                }
            }
